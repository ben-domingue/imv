...
```
We'll then fit a null model `m0` and a model with predictor `x` as `m1`.
```r
m0 <- glm(y ~ 1, df, family = "binomial")
m1 <- glm(y ~ x, df, family = "binomial")
```
We can compute the IMV directly on held-out predictions using `imv.binary()`. Here we generate a new set of outcomes `new.y` and use `m1` to predict them.
```r
new.y <- rbinom(length(x), 1, plogis(x))
pr    <- predict(m1, data.frame(x = x), type = "response")
imv.binary(new.y, mean(y), pr)
```
How should we interpret a value of `0.44`? In Table 1 [here](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0316491) we provide a range of values from empirical studies that this value could be compared to.

### Cross-validation and the IMV
Alternatively, we can compute a cross-validated IMV by passing both models to `imv()`. Each model is refit on each training fold and evaluated on the held-out fold.
```r
result <- imv(m0, m1)
result$mean
result$ci
```
The result is a list with per-fold values (`$folds`), the mean IMV (`$mean`), its standard deviation (`$sd`), and a 95% confidence interval (`$ci`).

### The IMV and individual predictors
We can use the IMV to quantify the contribution of an individual predictor by comparing a model with and without it. Here we add a predictor `z` that is unassociated with `y`.
```r
df$z <- rnorm(nrow(df))
m0   <- glm(y ~ x,     df, family = "binomial")
m1   <- glm(y ~ x + z, df, family = "binomial")
result <- imv(m0, m1)
result$mean
```
...
m0 <- glm(diabetes ~ mass + age,           PimaIndiansDiabetes, family = "binomial")
m1 <- glm(diabetes ~ glucose + mass + age, PimaIndiansDiabetes, family = "binomial")
result <- imv(m0, m1)
result$mean
```
...
imv(mod1)  # compared to item-level means
```
...
model <- mirt.model(s)
mod2  <- mirt(resp, model, itemtype = rep("2PL", ni), method = "EM",
              technical = list(NCYCLES = 10000), verbose = FALSE)
